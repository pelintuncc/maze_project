...
class MazePainter extends ChangeNotifier implements CustomPainter {
  ...
  MazePainter({
    ...
    required List<List<bool>> userDefinedWalls,
  }) : _userDefinedWalls = userDefinedWalls {
    _wallPaint
      ..color = wallColor
      ..isAntiAlias = true
    ...
    _checkpointsPositions = _checkpoints
        .map((i) => ItemPosition(
      col: _randomizer.nextInt(columns),
      row: _randomizer.nextInt(rows),
    ))
        .toList();
  ...
  /// Callback when the player reaches a checkpoint
  final Function(int)? onCheckpoint;

  /// Callback when the player reaches the finish
  ...
  /// User-defined walls
  List<List<bool>> _userDefinedWalls;

  /// Set user-defined walls dynamically
  void setUserDefinedWalls(List<List<bool>> userDefinedWalls) {
    _userDefinedWalls = userDefinedWalls;
    _createMaze();
    notifyListeners();
  }

  /// This method initializes the maze based on user-defined walls.
  void _createMaze() {
    ...
    _cells = List.generate(
      columns,
          (col) => List.generate(rows, (row) => Cell(col, row)),
    );
    ...
    // Apply user-defined walls
    for (int col = 0; col < columns; col++) {
      for (int row = 0; row < rows; row++) {
        if (_userDefinedWalls[row][col]) {
          _cells[col][row].topWall = true;
          _cells[col][row].bottomWall = true;
          _cells[col][row].leftWall = true;
          _cells[col][row].rightWall = true;
        }
      }
    }
  }
  ...
  /// This method moves the player to user input
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (finishImage != null) {
      canvas.drawImageRect(
        ...
        Offset.zero &
        Size(
          finishImage!.width.toDouble(),
          finishImage!.height.toDouble(),
        ),
        Offset(
            _exit.col * _cellSize + squareMargin,
      ...
      canvas.drawRect(
        Rect.fromPoints(
          ...
          Offset(
            (_exit.col + 1) * _cellSize - squareMargin,
    ...
    for (var i = 0; i < _checkpoints.length; i++) {
      canvas.drawImageRect(
        ...
        Offset.zero &
        Size(
          _checkpoints[i].width.toDouble(),
          _checkpoints[i].height.toDouble(),
        ),
        Offset(
            _checkpointsPositions[i].col * _cellSize + squareMargin,
    ...
    canvas.drawImageRect(
      ...
      Offset(
          _player.col * _cellSize + squareMargin,
